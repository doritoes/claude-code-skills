...
async function generateFeedback(options: {
  ...
  if (!dryRun) {
    ...
    // Load beta-exclusions.txt (generated by BetaCurator)
    const exclusionsPath = resolve(FEEDBACK_DIR, "beta-exclusions.txt");
    const exclusions = new Set<string>();
    if (existsSync(exclusionsPath)) {
      const content = readFileSync(exclusionsPath, "utf-8");
      for (const line of content.split("\n")) {
        const word = line.trim().toLowerCase();
        if (word && !word.startsWith("#")) exclusions.add(word);
      }
      console.log(`  Loaded ${exclusions.size.toLocaleString()} exclusions from beta-exclusions.txt`);
    }

    // Build BETA.txt: discovered + cohorts - exclusions
    const betaWords = new Set<string>();
    for (const root of discoveredRoots) {
      if (!exclusions.has(root)) betaWords.add(root);
    }

    let cohortWordsAdded = 0;
    let cohortExcluded = 0;
    if (existsSync(COHORTS_DIR)) {
      const cohortFiles = readdirSync(COHORTS_DIR).filter(f => f.endsWith(".txt"));
      for (const file of cohortFiles) {
        const cohortPath = resolve(COHORTS_DIR, file);
      ...
      console.log(`  Merged ${cohortWordsAdded.toLocaleString()} cohort words (${cohortExcluded.toLocaleString()} excluded by BetaCurator)`);
    }
  ...
  // Suggest curation if BETA.txt is large
  if (!dryRun && metricsBetaSize > 50_000) {
    console.log(`\n  ── CURATION SUGGESTION ──`);
    console.log(`  BETA.txt has ${metricsBetaSize.toLocaleString()} roots. Consider pruning dead roots.`);
