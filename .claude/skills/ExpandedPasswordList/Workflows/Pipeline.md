...
| `BetaCurator.ts` | **BETA.txt curation** — exclude dead cohort roots using crack statistics | `--execute`, `--min-cracks N` |
...
## Phase 5a: BETA.txt Curation (Periodic)

Prune dead roots from BETA.txt to speed up feedback attacks. Run every ~50 batches or when BETA.txt exceeds 50K roots. Uses substring matching (not just prefix) to properly credit roots found via rules, combinators, and reverse hybrids.

```bash
cd .claude/skills/ExpandedPasswordList

# Dry run — show cohort health table, exclusion counts, estimated speedup
bun Tools/BetaCurator.ts

# Apply — generates beta-exclusions.txt, rebuilds BETA.txt
bun Tools/BetaCurator.ts --execute

# More aggressive — exclude roots with < 3 cracks (use with larger sample sizes)
bun Tools/BetaCurator.ts --execute --min-cracks 3
```

**How it works:**
1. Streams all diamonds, counts cracks per BETA root via **substring match** (finds root anywhere in password, not just prefix — accounts for combinator suffix, reverse hybrid prefix, prepend rules)
2. Roots shorter than 4 chars are exempt from exclusion (can't be measured by substring match)
3. Generates `beta-exclusions.txt` — cohort source files are NEVER modified
4. Rebuilds BETA.txt = discovered + cohorts - exclusions
5. DiamondFeedback respects `beta-exclusions.txt` on subsequent BETA.txt rebuilds

**Output files:**
- `data/feedback/beta-exclusions.txt` — excluded roots (portable, re-evaluated on each run)
- `data/feedback/BETA.txt` — rebuilt without excluded roots

**After executing:** Sync updated BETA.txt to BIGRED: `bun Tools/BigRedSync.ts --sync-attack-files`

**Re-evaluation:** As more batches complete, some excluded roots may start producing cracks. Re-run BetaCurator periodically to re-include them.
...
| `BETA.txt` | `data/feedback/BETA.txt` | Discovered roots + cohort words - exclusions (rebuilds each batch via DiamondFeedback) |
| `beta-exclusions.txt` | `data/feedback/beta-exclusions.txt` | Dead roots excluded from BETA.txt (generated by BetaCurator, respected by DiamondFeedback) |
